...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const options = {
    ...
    secure: true,
    sameSite: "none",
    expiresIn: new Date(Date.now() + 360000000),
  };
  if (!createdUser) {
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const option = {
      ...
      secure: true,
      sameSite: "none",
      expiresIn: new Date(Date.now() + 3600000000),
    };
...
const getCurrentUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, req.user, "user was featched successfully"));
});
...
const google = async (req, res) => {
  try {
    ...
    if (user && exsistingUser) {
      ...
      const expiryDate = new Date(Date.now() + 36000000000); // 1 hour
      res
        .cookie("accessToken", accessToken, {
          httpOnly: true,
          expires: expiryDate,
